...
app.post("/generate-docs", async (req, res) => {
  try {
    const { apiKey, selectedFile, model = 'claude' } = req.body;
    ...
    if (model === 'claude') {
      // Claude API call
      result = await generateWithClaude(apiKey, selectedFile);
    } else if (model === 'gemini') {
      // Gemini API call
      result = await generateWithGemini(apiKey, selectedFile);
    } else {
...
async function generateWithClaude(apiKey, selectedFile) {
  ...
  const requestBody = {
    ...
    system: systemPrompt,
    messages: [{ role: "user", content: "Please generate documentation based on the provided JSON data." }],
  };
  ...
  return result.content?.[0]?.text || "No documentation generated.";
}

async function generateWithGemini(apiKey, selectedFile) {
  // Fix the API URL with the correct model name
  const ai = new GoogleGenAI({ apiKey: apiKey});
  
  const prompt = `##Response Grounding
    You are an AI assistant, You *should always* reference factual statements to search results based on documents.If the search results based on documents do not contain sufficient information to answer user message completely, you only use *facts from the search results* and ff the information is still not available in the documents below, reply with: 'Sorry, I could not find the information you were seeking. Can I help with something else? Try rephrasing the question.'
